...
\captionsetup[figure]{labelsep=period, labelfont=it, justification=raggedright, singlelinecheck=false}
\captionsetup[table]{textfont={it}, labelsep=newline, justification=raggedright, singlelinecheck=false}
...
$for(author)$
  \text{$author.name$} \\
  \text\small{$author.affiliation$} \\
  \text\small{$author.address$} \\
  \text\small{$author.email$} \\
  \vspace{5mm}
$endfor$
...
$if(acknowledgements)$
  ...
  \noindent\textit{Acknowledgements:} $acknowledgements$ \\
