#' @name add_yaml
#' @title Create or Update YAML
#' @author Nicolas Mangin
#' @description Gadget to insert the yaml of a research paper.
#' @return Write lines of the YAML at the top of the Rmd files in which the paper is written.
...
add_yaml <- function() {
  ...
  server <- function(input, output, session) {
    ...
    if (file.exists("dat/yamlinfo.RData")) {
      load("dat/yamlinfo.RData")
    } else {
      yamlinfo <- list(
        title = "",
        abstract = "",
        ...
        acknowledgements = "",
        authors = tibble(
          name = as.character(NA), email = as.character(NA),
    ...
    values$jelcodes <- writR::dat_jelcodes
    values$slctcodes <- writR::dat_jelcodes %>%
      dplyr::filter(code %in% yamlinfo$jel)
    values$journals <- writR::dat_journals
    ...
    output$cwabstr <- renderText({
      if (!is.null(input$abstract)) {
        paste0(tokenizers::count_words(input$abstract), " words.")
      } else {
        ""
      }
    })
    ...
    output$paperinfo <- renderUI({
      ui <- list(
        ...
        textAreaInput(
          "keywords",
          "Keywords (separated with comas):",
          value = yamlinfo$keywords,
          height = "50px"
        ) %>%
          shiny::tagAppendAttributes(style = "width: 100%;"),
        textAreaInput(
        ...
        textAreaInput(
          "acknowledgements",
          "Acknowledgements:",
          value = yamlinfo$acknowledgements,
          height = "75px"
        ) %>%
          shiny::tagAppendAttributes(style = "width: 100%;")
      )
    ...
    output$orgauthor <- renderUI({
      ui <- list(
        fluidRow(
          column(
            6,
            selectInput("authorrank", "Authors in order of apparition:",
              choices = values$authors$name,
              selected = values$authors$name,
              multiple = TRUE
            )
          ),
          column(4, selectInput("corresponding", "Corresponding author:",
            choices = values$authors$name,
            selected = values$authors$name[[1]],
            multiple = FALSE
          )),
      ...
      if (!is.null(input$corresponding)) {
        if (!is.null(input$authorrank)) {
          values$authors <- values$authors %>%
            ...
            dplyr::mutate(
              rank = as.integer(factor(name, levels = input$authorrank))
            ) %>%
            dplyr::arrange(rank)
        }
      }
    ...
    output$displayauthor <- DT::renderDT({
      values$authors
    })
    ...
    output$slctjelcodes <- renderUI({
      ...
      selectInput(
        "slctjel",
        "Select the following JEL codes",
        choices = choices,
        selected = selected,
        multiple = TRUE
      )
    ...
    output$slctjnl <- renderUI({
      ...
      ui <- fluidRow(
        ...
        column(
          3,
          textInput("submissionid", "Submission ID (if any):", value = "")
        )
      )
    })
        ...
        if (input$tgtjnl != "") {
          ...
          if (tgtjnl$type[[1]] == "writR") {
            ui <- list(
              ...
              checkboxInput(
                "tabfigtoend",
                "Place tables and figures at the end?",
                value = TRUE
              ),
              checkboxInput(
                "doublespace",
              ...
              checkboxInput(
                "raggedright",
                "Ragged right? (by default the text is justified)",
                value = FALSE
              )
            )
          } else {
    ...
    observeEvent(input$done, {
      ...
      save(yamlinfo, file = "dat/yamlinfo.RData")
      ...
      if (type == "elsevier") {
        ...
        for (i in 1:nrow(values$authors)) {
          ...
          if (tmp1$corresponding[[1]]) {
            tmp2 <- c(tmp2, "    footnote: Corresponding author.\n")
          }
        ...
        mdoutput <- c(
          ...
          paste0("     keep_tex: ", yamlinfo$keep_tex, "\n")
        )
